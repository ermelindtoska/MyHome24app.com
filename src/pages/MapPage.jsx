...
// 🔹 KJO ËSHTË E RE: Provider-i i search-state
import { SearchStateProvider } from '../state/useSearchState';

// --- util: geocode me Mapbox (DE) ---
...
async function geocodeDE(q) {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn('[MapPage] Mapbox geocoding HTTP error:', res.status, res.statusText);
      return null;
    }
    ...
    console.error('[MapPage] Mapbox geocoding error:', err);
  }
...
const MapPage = ({ purpose = 'all' }) => {
    ...
    const load = async () => {
      try {
        ...
        const filtered =
          purpose === 'all'
            ? rows
            : rows.filter((r) => r.purpose === purpose);
        ...
        if (filtered.length && withoutCoords) {
          console.warn(
            `[MapPage] ${withoutCoords}/${filtered.length} Listings ohne Koordinaten – versuche Geocoding.`
          );
        }

        setAllListings(filtered);

        // --- Geocode për listimet pa koordinata & ruaji në Firestore ---
  ...
  // 🔹 KTU E MBËSHTJELLIM GJITHË FAQEN ME SearchStateProvider
  return (
    <SearchStateProvider>
            ...
            <div className="sticky top-0 z-10 bg-white/90 dark:bg-gray-900/90 backdrop-blur border-b border-gray-200 dark:border-gray-700 px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <div className="text-sm text-gray-500 dark:text-gray-400">
                  {count}{' '}
                  {t('results', { defaultValue: 'Ergebnisse' })}
                </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 max-h-[75vh] bg-white dark:bg-gray-900 rounded-t-2xl shadow-2xl">
              <div className="p-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                <span className="font-semibold">
                  {t('results', { defaultValue: 'Rezultate' })} (
