// GermanyMapReal.jsx – Zillow-style interactive map with Listing Modal integration
...
const GermanyMapReal = () => {
  const { t } = useTranslation(['listing', 'filterBar']);
  const location = useLocation();
  const pathname = location.pathname;
  const inferredPurpose = pathname.includes('/buy') ? 'buy' : pathname.includes('/rent') ? 'rent' : null;
  ...
  // Këtu krijohet funksioni që mund të përdoret edhe poshtë
  const applyFilters = (items) => {
    return items.filter(item => {
      return (
        (!inferredPurpose || item.purpose === inferredPurpose) &&
  ...
  const filteredListings = applyFilters(listings); // tani është jashtë funksionit dhe në rregull ✅
  ...
  return (
    <div className="w-full h-[calc(100vh-64px)] flex flex-col lg:flex-row">
      {/* Filter Panel */}
      <div className="w-full lg:w-1/3 overflow-y-auto bg-white p-4 shadow-md">
        ...
        {activeListings.length === 0 ? (
          ...
          activeListings.map((item) => (
            ...
            >
              ...
              <p className="text-xs text-gray-400">{item.type} · {item.bedrooms} {t('bedrooms', { ns: 'listing' })}</p>
              <p className="text-gray-600 mt-2">{t('resultsTitle', { ns: 'filterBar' })} ({filteredListings.length})</p>
            </div>
          ))
        )}
      </div>
      ...
      {/* Map Panel */}
      <MapContainer center={[51.1657, 10.4515]} zoom={6} className="w-full lg:w-2/3 h-[50vh] lg:h-full z-0">
        <TileLayer
