...
const ComparePage = () => {
  ...
  const copySummary = () => {
    const summary = listings.map(item => `${item.title} - ${item.city} - ${formatPrice(item.price)}€`).join('\n');
  ...
  return (
    <div className={`${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'} min-h-screen pt-20 px-4 print:px-0`}>
      <nav className="sticky top-0 z-50 bg-white dark:bg-gray-800 border-b border-gray-300 dark:border-gray-700 shadow text-sm text-gray-700 dark:text-gray-200 py-2 px-4 flex items-center gap-2 print:hidden transition-all duration-300 ease-in-out">
        ...
        <button onClick={() => setFiltersVisible(!filtersVisible)} className="text-blue-600 hover:text-blue-800 flex items-center gap-1 ml-auto"><FaFilter /> {t('actions.toggleFilters')}</button>
        <button onClick={copySummary} className="text-blue-600 hover:text-blue-800 underline flex items-center gap-1 transition duration-200"><FaCopy /> {t('actions.copy')}</button>
      ...
      {filtersVisible && (
        <div className="bg-white dark:bg-gray-800 p-6 shadow rounded-md print:hidden mb-6">
          <div className="flex flex-wrap gap-4 items-center">
            <input type="text" placeholder={t('filters.city')} className="border px-3 py-2 rounded w-40 hover:border-blue-500" value={cityFilter} onChange={(e) => setCityFilter(e.target.value)} />
            <input type="number" placeholder={t('filters.price')} className="border px-3 py-2 rounded w-40 hover:border-blue-500" value={maxPriceFilter} onChange={(e) => setMaxPriceFilter(e.target.value)} />
            <input type="number" placeholder={t('filters.rooms')} className="border px-3 py-2 rounded w-40 hover:border-blue-500" value={minRatingFilter} onChange={(e) => setMinRatingFilter(e.target.value)} />
            <input type="number" placeholder={t('filters.size')} className="border px-3 py-2 rounded w-40 hover:border-blue-500" value={minSizeFilter} onChange={(e) => setMinSizeFilter(e.target.value)} />
            <input type="number" placeholder={t('filters.size')} className="border px-3 py-2 rounded w-40 hover:border-blue-500" value={maxSizeFilter} onChange={(e) => setMaxSizeFilter(e.target.value)} />
            <button className="text-blue-600 hover:text-blue-800 underline transition duration-200" onClick={resetFilters}>{t('actions.resetFilters')}</button>
            <button onClick={refreshData} className="text-blue-600 hover:text-blue-800 underline flex items-center gap-1 transition duration-200"><FaSync /> {t('actions.refresh')}</button>
            <button onClick={toggleDarkMode} className="text-yellow-600 hover:text-yellow-400 underline flex items-center gap-1 transition duration-200">{darkMode ? <FaSun /> : <FaMoon />} {darkMode ? t('actions.lightMode') : t('actions.darkMode')}</button>
            <button onClick={clearAllListings} className="text-red-600 hover:text-red-800 underline flex items-center gap-1 transition duration-200"><FaTrash /> {t('actions.clearAll')}</button>
          </div>
        </div>
      )}
