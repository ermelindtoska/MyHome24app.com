  ...
  return (
    <div className="bg-white p-4 rounded shadow-md w-full max-w-md mx-auto mt-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        <input name="title" value={form.title} onChange={handleChange} placeholder={t('title')} className="w-full p-2 border rounded" />
        <input name="city" value={form.city} onChange={handleChange} placeholder={t('city')} className="w-full p-2 border rounded" />
        ...
        <button type="submit" className="w-full bg-blue-600 text-white p-2 rounded">{t('submit')}</button>
      </form>
    </div>
  );
};
